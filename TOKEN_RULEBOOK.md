...
| Aspect Ratio, Direction, Input OTP, Native Select, Resizable, Carousel, Chart, Menubar | Geometry/Color/Font | Kept exactly as shadcn's default for all 8 — no changes needed. Seventh and final application of the default policy for this pass through the catalog. | batch-7 milestone |
| Native Select | Color (special case, not an exception) | `<option>`/`<optgroup>` intentionally use CSS system-color keywords (`Canvas`/`CanvasText`), not brand hex or a CSS variable — browsers don't allow custom styling of native `<select>` popup content, so this is the correct, standard approach, not a policy violation. | batch-7 milestone |
| Chart | Color (documented boundary) | Chrome (grid lines, tooltip background, legend text) uses `--border`/`--background`/`--muted-foreground`, Figma-sourced as usual. Per-series *data* colors (the bars/lines themselves) are supplied by the calling code via `ChartConfig`, not by this component or Figma — no chart-color token set exists in Figma yet. Not a gap in this component; a future scope item if per-chart brand colors are wanted. | batch-7 milestone |
| Sidebar | Color (known gap, not new) | Uses shadcn's own `--sidebar-*` variable set (background/foreground/accent/border/ring/primary), already defined in `src/index.css` as shadcn's greyscale placeholders. This is the exact gap already listed in Section 3 ("Sidebar colors") — flagged again here only because this is the first component that actually surfaces it, not because anything new was discovered. | batch-7 milestone |
| *(shadcn's `typography`)* | Not a component | Not added — it's a docs page of example prose `className` combinations (`prose`, heading/paragraph styles), not a copyable component file. No registry entry makes sense for it. | batch-7 milestone |
| *(all 56 components)* | Catalog completion | This closes out the full non-chat shadcn catalog per user decision. Remaining out-of-scope by choice: Bubble, Message, Message Scroller, Marker, Attachment (chat-specific), Toast (superseded by Sonner). Remaining out-of-scope because they aren't real registry items: Date Picker, Data Table, Typography (see their individual rows above). | batch-7 milestone |
